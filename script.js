{
	const si={
		...
		init(){
			si.items=document.querySelectorAll(si.selectors.item);
			si.mode.init();
			si.sort.init();
			si.search.init();
		...
		getsvg(item,fill){
			let svg=item.querySelector(si.selectors.icon).cloneNode(true);
		...
		handler(event){
			...
			if(target){
				let	data=target.dataset,
					item=target.closest(si.selectors.item);
				switch(true){
					case data.copy===`hex`:
						si.copy(target,si.getcolor(item));
						break;
					case data.copy===`npm`:
						si.copy(target,target.firstChild.firstChild.nodeValue);
						break;
					case data.copy===`svg`:
		...
		search:{
			...
			init(){
				...
				si.search.query=si.url.searchParams.get(si.search.param);
				if(si.search.query){
			...
			filter(value=``,init=false){
				...
				if(value){
					if(!init)
						si.url.searchParams.set(si.search.param,value);
					if(si.sort.auto)
					...
					si.request.classList.toggle(`hide`,!noresults);
					si.normalised=true;
				}else{
					si.url.searchParams.delete(si.search.param);
